...
namespace Drive
{
    public class CContour
    {
        ...
        public double rEastCT, rNorthCT, eCat, nCat;
        ...
        public void BuildContourGuidanceLine(vec3 pivot)
        {
            ...
            double distSq = widthMinusOverlap * widthMinusOverlap * 0.9999999999999;
            ...
            for (int i = start; i < stop; i++)
            {
                ...
                if (Add)
                {
                    if (ctList.Count > 0)
                    {
                        double dist = ((point.easting - ctList[ctList.Count - 1].easting) * (point.easting 
                            - ctList[ctList.Count - 1].easting)) + ((point.northing - ctList[ctList.Count - 1].northing) 
                            * (point.northing - ctList[ctList.Count - 1].northing));
                        if (dist > 2)
                            ctList.Add(point);
                    }
            ...
            ////make sure not too many close points
            //double spacing = 0.4;
            //double distance;
            ...
            ctList.Add(arr[cnt - 2]);
            ctList.Add(arr[cnt - 1]);

            //final heading calc
            CalculateContourHeadings();
        }
        ...
        public void DistanceFromContourLine(vec3 pivot, vec3 steer)
        {
            ...
            if (ptCount > 8)
            {
                if (mf.isStanleyUsed)
                {
                    ...
                    {
                        double dist = ((steer.easting - ctList[k].easting) * (steer.easting - ctList[k].easting))
                                        + ((steer.northing - ctList[k].northing) * (steer.northing - ctList[k].northing));
                        if (dist < minDistA)
                        {
                            minDistB = minDistA;
                            B = A;
                    ...
                    eCat = ctList[A].easting + (U * dx);
                    nCat = ctList[A].northing + (U * dy);

                    double t =  glm.Distance(ctList[A], eCat, nCat) / glm.Distance(ctList[B], ctList[A]);

                    if (A > 1 && B < ptCount-2)
                    {
                        vec3 pos = new vec3(glm.Catmull(t, ctList[A-1], ctList[A], ctList[A+1], ctList[A+2]));
                        eCat = pos.easting;
                        nCat = pos.northing;
                    }
                    ...
                    steerAngleCT = Math.Atan((distanceFromCurrentLine * mf.ast.stanleyGain) 
