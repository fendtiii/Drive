    ...
    public partial class FormGPS
    {
        ...
        private void TheRest()
        {
            ...
            if (bnd.isOkToAddPoints)
            {
                ...
                if (boundaryDistance > 2) AddBoundaryPoint();
            }

            if (curve.isOkToAddPoints)
            {
                double cuDist = 0;
                if (curve.refList.Count > 1)
                {
                    cuDist = glm.Distance(pn.fix, curve.refList[curve.refList.Count - 1]);
                    if (cuDist > 2)
                    {
        ...
        private void CalculatePositionHeading()
        {
            ...
            if (tool.isToolTrailing)
            {
                ...
                if (distanceCurrentStepFix != 0)
                {
                    ...
                    toolPos.easting += t * (tankPos.easting - toolPos.easting);
                    toolPos.northing += t * (tankPos.northing - toolPos.northing);
        ...
        private void AddSectionOrContourPathPoints()
        {
            ...
            else
            {
                ...
                //Build contour line if close enough to a patch - done every 3 seconds in gui.cs
