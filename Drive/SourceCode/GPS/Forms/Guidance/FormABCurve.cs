    ...
    public partial class FormABCurve : Form
    {
        ...
        private void btnBPoint_Click(object sender, System.EventArgs e)
        {
            ...
            if (cnt > 5)
            {
                ...
                mf.curve.refList.Add(arr[0]);
                mf.curve.refList.Add(arr[1]);

                for (int i = 2; i < cnt - 4; i++)
                {
                    // add p1
                    mf.curve.refList.Add(arr[i + 1]);

                    for (int j = 1; j < 6; j++)
                    {
                        vec3 pos = new vec3(glm.Catmull(j / 6.0, arr[i], arr[i + 1], arr[i + 2], arr[i + 3]));
                        mf.curve.refList.Add(pos);
                    }
                }

                mf.curve.refList.Add(arr[cnt-2]);

                mf.curve.refList.RemoveRange(0, 2);

                mf.curve.CalculateTurnHeadings();

               mf.curve.AddFirstLastPoints(20);
