...
namespace Drive
{
    public partial class FormGPS
    {
        ...
        private bool isBoundaryClose = false, isMapping = true;
        ...
        private void oglBack_Paint(object sender, PaintEventArgs e)
        {
            ...
            if ((tool.isRightSideInHeadland || tool.isLeftSideInHeadland))
                tool.isSuperSectionAllowedOn = false;
            ...
            if ((rpOnHeight < rpToolHeight)) rpHeight = rpToolHeight + 2;
            ...
            if (minuteCounter > 30 && sentenceCounter < 20)
            {
                ...
                if (isLogHeading) FileSaveHeading();
        ...
        private void DrawLightBarText()
        {
            ...
            if (ct.isContourBtnOn || ABLine.isBtnABLineOn || curve.isBtnCurveOn)
            {
                ...
                if (dist != 3200 && dist != 3202)
                {
                    ...
                    //dist = glm.toDegrees(ABLine.abFixHeadingDelta);

                    //size = 1;

                    //if (dist > 0.0)
                    //{
                    //    GL.Color3(0.9752f, 0.20f, 0.63f);
                    //    hede = "< " + (Math.Abs(dist)).ToString("N1");
                    //}
                    //else
                    //{
                    //    GL.Color3(0.20f, 0.952f, 0.83f);
                    //    hede = (Math.Abs(dist)).ToString("N1") + " >";
                    //}
                    
                    //center = -(int)(((double)(hede.Length) * 0.5) * 16 * size);

                    //font.DrawText(center, 80, hede, size);
                }
            }
